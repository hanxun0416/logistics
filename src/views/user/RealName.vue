<template>
    <div class="real" id="real">
       ...
       <div class="real-mid-field">
        <van-field v-model="applyTrans.corp"  label="公司名称" class="real-mid-field-inp" placeholder="请输入公司名称" type="text" clearable name="applyTrans.corp" required> </van-field>
        <van-field
           label="公司税号"
...
/>
      ...
      <!-- 个人信息 -->
       ...
       <div class="real-mid-field">
        <van-field 
        v-model="applyTrans.custContact"  
        label="联系人" 
        class="real-mid-field-inp" 
        placeholder="请输入联系人"
        name="applyTrans.custContact"
         type="text" clearable 
         required> 
         </van-field>
        <van-field
           label="手机号"
          v-model="applyTrans.custMobile"
          class="real-mid-field-inp"
          placeholder="请输入手机号"
          type="text"
          name="applyTrans.custMobile"
          clearable
          required
        >
        </van-field>
        ...
        >
         ...
         <van-field
          label="推荐金牌商家"
          :value="custMap.fromUserName"
          class="real-mid-field-inp"
          placeholder="请选择金牌商家"
          type="text"
          name="costomer.fromUserName"
          readonly
          clickable
          @click="show10"
        > </van-field>
       <van-overlay :show="isShow2">
  <div class="wrapper">
    <van-search placeholder="请输入搜索关键词" v-model="image.custName"   show-action @cancel="clickLeft"     @input="sceneReview"/>
    <div class="sear">
      <div class="sear-card" @click="clickHandle(item.custName,item.custID)" v-for="item in imageList" :key="item.id">
  ...
  <van-col span="12" class="real-mid-field-upload-right1"  @click="downLoad(1)">下载运输合同</van-col>
   <van-col span="12" class="real-mid-field-upload-right1"  @click="downLoad(2)">下载订舱合同</van-col>
       ...
       <van-uploader
        ...
        :max-count="10"
        upload-text="添加图片"  @oversize="oversize" multiple/>
         <div class="maxsize">注:最多添加10张</div></div>
        </div>
      </div>
      <!-- 选择现场评审人员 -->
      <van-field
...
/>
<van-action-sheet v-model="isShow1" :actions="actions" @select="onSelect"   cancel-text="取消"/>
       <van-overlay :show="a">
  <div class="wrapper">
    <van-search placeholder="请输入搜索关键词" v-model="image.custName" @cancel="clickLeft"    show-action   @input="sceneReview"/>
    <div class="sear">
...
export default {
    ...
    methods: {
      afterRead(item,index){
         console.log(this.trad1.length)
        if(this.trad1.length>=10){
          Toast("最多上传20张")
        }
      },
      show10(){
...
clickHandle1(name,id){
  ...
  this.a=false
    ...
    //金牌商家选择
clickHandle(name,id){
  this.custMap.fromUserId=id
  this.custMap.fromUserName=name
  this.imageList=[]
  this.image.custName=""
  this.isShow2=false
},
...
changeAddress(value){
    ...
    submit(){
     ...
     if(this.applyTrans.custContact.trim()==""){
       Toast.fail('请输入联系人!')
       return;
     }
     if(this.applyTrans.custMobile.trim()==""||!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(this.applyTrans.custMobile))){
    ...
    if(this.applyTrans.bankAccount.trim()==""){
      ...
      if(this.custMap.cardNO.trim()==""||!(/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(this.custMap.cardNO))){
       Toast.fail('请输入正确的身份证号')
       return;
     }
     if(!this.trad1.length||!this.trad2.length||!this.trad3.length||!this.trad4.length||!this.trad5.length||!this.trad1.length||!this.trad6.length){
           Toast.fail('请上传合同！')
           return;
     }
        let imgList = [];
  ...
  for (let index = 0; index < this.trad4.length; index++) {
       ...
       //运输合同/订舱合同
           for (let index = 0; index < this.trad6.length; index++) {
         let imgObj = {
          content: this.trad6[index].content,
          fileName: this.trad6[index].file.name,
          nodeID: "1",
          tabletdID: "6",
          sceneType: "2",
          tableName: "lg_pickbillmain",
    ...
    upload(){
      if(this.applyTrans.isSelect=='1'){
        ...
        'applyTrans.custContact':this.applyTrans.custContact ,//联系人
        'applyTrans.custMobile':this.applyTrans.custMobile,//手机号
        ...
        'custMap.cardNO':this.custMap.cardNO,//身份证号
        ...
        'custMap.fromUserName':this.custMap.fromUserName,//金牌商家姓名
        'custMap.fromUserId':this.custMap.fromUserId,//金牌商家ID
      }
